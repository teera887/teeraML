...
def load_and_split_dataset(df, feature_columns, target_column):
...
# Train LSTM Model
def train_lstm_model(X_train_scaled, y_train, epochs=50, batch_size=32):
...
db_params = {
    "host": "portfolio-insider-production.cdawbatxpujr.us-east-1.rds.amazonaws.com",
    "database": "pi",
    "user": "postgres",
    "password": "portfolio.insider",
    "port": "5432"
}
...
def main():
    ...
    try:
        ...
        with psycopg2.connect(**db_params) as conn:
            ...
            query = f"SELECT DISTINCT ticker FROM {table_name} WHERE delisted = false"
            all_symbols = fetch_symbols_from_database(cursor, query)

            # Filter symbols
            symbols_of_choice = ['ARM', 'NVDA', 'AAPL', 'TSGS']
            filtered_symbols = [symbol for symbol in all_symbols if symbol in symbols_of_choice]

            if not filtered_symbols:
                print("No symbols to process based on your filter.")
                return
